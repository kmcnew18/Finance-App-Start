...
function classifyTxnType(txn) {
  ...
  // A genuine deposit — cash/check/ATM deposit, or a loan/cash-advance
  // hitting the account — still falls under Plaid's broader TRANSFER_IN
  // primary category below, even though it's real money coming in from
  // outside, not a movement between two of the user's own accounts.
  ...
  // deposited into a bank account" — as opposed to
  // TRANSFER_IN_ACCOUNT_TRANSFER ("general inbound transfers from
  // another account"), which is the one that's actually transfer-shaped
  // and stays classified as a transfer below. Checked first, same
  // reasoning as the credit-card-payment override right after it.
  if (detailed === 'TRANSFER_IN_DEPOSIT' || detailed === 'TRANSFER_IN_CASH_ADVANCES_AND_LOANS') {
