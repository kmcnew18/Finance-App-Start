...
async function isLikelyReimbursedPattern(userId, merchantName) {
  ...
  if (!txns || txns.length < 4) return false; // too little history to judge reliably
...
async function refreshRecurringForItem(itemRow) {
  ...
  for (const s of streams) {
    ...
    const isReimbursedPattern = !isTombstoned && await isLikelyReimbursedPattern(itemRow.user_id, s.merchant_name || s.description);
...
async function queueDashboardReviews(userId, addedTransactions) {
  ...
  for (const txn of addedTransactions) {
    ...
    // a general self-transfer (checking -> savings) is still a real,
    // active choice worth logging, tagged as a transfer rather than an
    // add/subtract, matching how the manual Transfer panel already
    // writes to the log. A credit card bill payment specifically is
    // different: the original purchase was already accounted for the
    // moment it happened, so the payment itself is just debt being
    // settled, not a decision — it stays purely internal, detected only
    // so it doesn't also get suggested as a double-counted expense.
    if (isCreditCardPayment(txn)) continue;
    ...
    if (accountTypeById[linkedAccountId] === 'credit_card') {
      ...
      categoryId = singleCategoryByAccountId[linkedAccountId];
...
async function processItemUpdate(itemRow) {
  ...
  // Only checking (debit) and credit card accounts — investment trades
  // and loan payments don't actually fit an "add/subtract to a
  // category" model the way a purchase does, and trying to force them
  // through the same review flow just added noise without being
  // genuinely useful.
  ...
  const cardEligible = rawAdded.filter(t => eligiblePlaidAccountIds.has(t.account_id));

  // Two different cutoffs, on purpose:
  //
  // Storage (feeds Spendings' history/charts) — bounded to the previous
  // calendar month onward, computed once from the connection date. This
  // lets Spendings show "this month + last month" even if Plaid
  // delivers some of that history asynchronously after the initial
  // connection, rather than storage staying permanently empty for
  // anything that didn't arrive in the very first sync.
  ...
  // Dashboard review — strictly the moment of connection forward, no
  // exceptions. A transaction that happened before the account was
  // connected should never show up as something "new" to review here,
  // regardless of when Plaid actually delivers it.
  const dashboardEligible = connectedDate ? cardEligible.filter(t => t.date >= connectedDate) : cardEligible;
  ...
  const dashboardIds = dashboardEligible.map(t => t.transaction_id);
  ...
  // Stamp every account under this connection as freshly synced — this
  // is what Connections' "Last synced" display reads from, so it needs
  // to happen every time this function actually completes a real sync,
  ...
  return { addedCount: storageEligible.length, queuedCount, dashboardQueuedCount };
}
