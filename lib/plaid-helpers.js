...
// Plaid re-issues a transaction_id when a pending transaction posts — the
// pending version comes back in `removed`, and a brand-new added
// transaction (different id, same real-world charge) takes its place. If
// the pending version was already queued for review, that queue row is
// now stale: the same deposit/charge is about to be queued again under
// its new id, which is what shows up as an exact duplicate in Detected
// activity. Clearing out any still-pending review tied to a removed
// transaction id avoids that — reviews already approved or dismissed are
// left alone, since the user already made a decision on those.
async function cleanupRemovedReviews(userId, removedTransactions) {
  if (!removedTransactions || !removedTransactions.length) return;
...
async function refreshTransactionsForItem(itemRow) {
  ...
  await cleanupRemovedReviews(itemRow.user_id, removedTransactions);
