...
function isSyncDue(itemRow) {
  if (!itemRow.last_sync_at) return true;
  return (Date.now() - new Date(itemRow.last_sync_at).getTime()) >= SYNC_INTERVAL_MS;
}

function nextSyncAt(itemRow) {
  if (!itemRow.last_sync_at) return null;
  return new Date(new Date(itemRow.last_sync_at).getTime() + SYNC_INTERVAL_MS).toISOString();
}
...
async function refreshTransactionsForItem(itemRow) {
  if (!isSyncDue(itemRow)) {
    return { addedCount: 0, dashboardQueuedCount: 0, merchantQueuedCount: 0, skipped: true, nextSyncAt: nextSyncAt(itemRow) };
  }
...
async function refreshSubscriptionsForItem(itemRow) {
  if (!isSyncDue(itemRow)) {
    return { streamCount: 0, queuedCount: 0, skipped: true, nextSyncAt: nextSyncAt(itemRow) };
  }
...
// the other. Both halves already respect the shared once-a-day gate
// individually, so this needs no gate logic of its own — it just
// bubbles up whether the item was actually due.
