...
async function verifyCrossAccountTransfer(userId, txn, linkedAccountId) {
  ...
  const { data: candidates } = await supabaseAdmin
    .from('transactions')
    .select('id, amount, linked_account_id')
    .eq('user_id', userId)
    .eq('is_removed', false)
    .neq('linked_account_id', linkedAccountId)
    .gte('txn_date', windowStart.toISOString().slice(0, 10))
    .lte('txn_date', windowEnd.toISOString().slice(0, 10));

  return (candidates || []).some(c => {
    const sameMagnitude = Math.abs(Math.abs(Number(c.amount)) - Math.abs(txn.amount)) < 0.01;
    const oppositeSign = (Number(c.amount) > 0) !== (txn.amount > 0);
    return sameMagnitude && oppositeSign;
  });
}

// Same as classifyTxnType, but for the 'transfer' case specifically,
// also confirms it against verifyCrossAccountTransfer before trusting
// Plaid's label — see that function's comment for why. Needs
// linkedAccountId (which caller must have already resolved) and is
// async because the verification is a DB lookup. Credit card payments
// skip verification entirely, since paying down a connected card is
// inherently a self-transfer by definition.
async function classifyTxnTypeVerified(userId, txn, linkedAccountId) {
  ...
  const confirmed = await verifyCrossAccountTransfer(userId, txn, linkedAccountId);
  return confirmed ? 'transfer' : (txn.amount > 0 ? 'expense' : 'income');
}
...
// instead of pending_dashboard_reviews. This one specifically matters
// because verifyCrossAccountTransfer has a genuine race condition: if
// the "other side" of a real transfer hadn't been synced yet at the
// moment this side was being classified, verification fails and the
// transaction gets stored as a plain expense/income — permanently,
// since nothing re-checks it once the pair actually does show up.
// Spendings filters on txn_type = 'transfer' to exclude these, so a
// transaction stuck with a stale, incorrect classification shows up
// there indefinitely even after its pair has long since arrived.
async function reclassifyStoredTransactions(userId) {
  const { data: txns } = await supabaseAdmin
    .from('transactions')
    .select('id, amount, txn_date, plaid_category, plaid_detailed_category, txn_type, linked_account_id')
    .eq('user_id', userId)
    .eq('is_removed', false)
...
async function reclassifyPendingReviews(userId) {
  ...
  for (const review of pending) {
    ...
    let nowIsTransfer = classifyTxnType(pseudoTxn) === 'transfer';
    if (nowIsTransfer && !nowIsCreditCardPayment) {
      nowIsTransfer = await verifyCrossAccountTransfer(userId, pseudoTxn, review.linked_account_id);
    }
