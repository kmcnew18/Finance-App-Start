...
async function queueDashboardReviews(userId, addedTransactions) {
  ...
  for (const txn of addedTransactions) {
    // Transfers (moving money between the user's own accounts — most
    // commonly a credit card bill payment) aren't real income/spending;
    // the actual purchase was already categorized when the card was
    // charged. Suggesting the bill payment too would double-count it.
    if (classifyTxnType(txn) === 'transfer') continue;
