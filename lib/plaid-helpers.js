...
// ================= ONCE-A-DAY SYNC GATE =================
//
// A single shared daily budget per Plaid Item, covering every kind of
// live Plaid call — balance refreshes (plaid-sync-accounts.js),
// transaction syncs, and subscription/recurring refreshes (both here).
// Whichever of those runs first for an item in a given 24-hour window
// marks it synced via markItemSynced(); every other call for that same
// item — whether it came from a button click, a page load, or Plaid's
// own webhook — sees isSyncDue() return false and skips its Plaid call
// entirely until the window resets. This is the one place that
// decision gets made, so nothing can bypass it by going through a
// different endpoint.
//
// Requires the plaid_items table to have a last_sync_at timestamptz
// column: alter table plaid_items add column if not exists
// last_sync_at timestamptz;
const SYNC_INTERVAL_MS = 24 * 60 * 60 * 1000;

// Plaid's own webhook (SYNC_UPDATES_AVAILABLE) fires the moment new
// transaction data is actually available at the bank — that's a real,
// externally-triggered event, not a user hammering refresh, so gating
// it behind the same once-a-day budget as a button click or page load
// was needlessly delaying detection by up to 24 hours for exactly the
// case that matters most: a new subscription's first charge sitting
// undetected until whenever the user next happened to visit. A
// webhook-triggered sync now bypasses the daily gate (see the `force`
// option below) — but still respects this much shorter floor, so a
// burst of webhooks for the same item (Plaid can send more than one in
// quick succession) can't turn into a spam of live Plaid calls.
const FORCE_SYNC_MIN_INTERVAL_MS = 15 * 60 * 1000;

function isSyncDue(itemRow, { force = false } = {}) {
  if (!itemRow.last_sync_at) return true;
  const elapsed = Date.now() - new Date(itemRow.last_sync_at).getTime();
  if (force) return elapsed >= FORCE_SYNC_MIN_INTERVAL_MS;
  return elapsed >= SYNC_INTERVAL_MS;
}

function nextSyncAt(itemRow, { force = false } = {}) {
  if (!itemRow.last_sync_at) return null;
  const interval = force ? FORCE_SYNC_MIN_INTERVAL_MS : SYNC_INTERVAL_MS;
  return new Date(new Date(itemRow.last_sync_at).getTime() + interval).toISOString();
}

async function markItemSynced(itemId) {
  await supabaseAdmin.from('plaid_items').update({ last_sync_at: new Date().toISOString() }).eq('item_id', itemId);
}
...
// Insert/update one recurring_streams row, tolerating a not-yet-migrated
// plaid_account_id column: PostgREST rejects the entire write when a
// payload references a column the table doesn't have (error 42703), so
// without this, one missing column would silently stop every stream from
// syncing rather than just missing the one new field. On that specific
// error, retries once with plaid_account_id stripped out; any other
// error is logged and left as-is rather than retried blindly.
async function upsertStream(existing, fields) {
  const query = existing
    ? supabaseAdmin.from('recurring_streams').update(fields).eq('id', existing.id)
    : supabaseAdmin.from('recurring_streams').insert(fields);
  const { error } = await query;
  if (!error) return;

  if (error.code === '42703') {
    const { plaid_account_id, ...fieldsWithoutAccountId } = fields;
    const retryQuery = existing
      ? supabaseAdmin.from('recurring_streams').update(fieldsWithoutAccountId).eq('id', existing.id)
      : supabaseAdmin.from('recurring_streams').insert(fieldsWithoutAccountId);
    const { error: retryError } = await retryQuery;
    if (retryError) console.error('recurring_streams write failed even without plaid_account_id:', retryError);
    return;
  }

  console.error('recurring_streams write failed:', error);
}
...
async function refreshRecurringForItem(itemRow) {
  ...
  for (const s of streams) {
    ...
    const fields = {
      ...
      // Which account this stream's charges actually land on — Plaid
      // includes this on every stream object, but nothing was ever
      // storing it, so Spendings had no way to tell (and therefore never
      // flagged) a subscription billed to a credit card versus one
      // pulled straight from checking. Requires the recurring_streams
      // table to have a plaid_account_id text column: alter table
      // recurring_streams add column if not exists plaid_account_id text.
      plaid_account_id: s.account_id || null,
    ...
    // plaid_account_id needs the migration noted above — until that's
    // run, PostgREST rejects the whole write for an unrecognized
    // column (not just that one field), which would otherwise silently
    // stop every subscription from syncing at all, not just lose the
    // new card badge. Falling back to writing without it keeps normal
    // syncing working in that window; upsertStream itself logs the
    // failure either way so a real problem doesn't go unnoticed.
    await upsertStream(existing, fields);
  }
...
function classifyTxnType(txn) {
  ...
  // A genuine deposit — cash/check/ATM deposit, a loan/cash-advance
  // hitting the account, or an inbound/outbound transaction Plaid
  // couldn't pin down more specifically — still falls under Plaid's
  // broader TRANSFER_IN/TRANSFER_OUT primary category below, even
  // though it's real money moving in or out from outside, not
  // necessarily a movement between two of the user's own accounts.
  // Confirmed against Plaid's own taxonomy: TRANSFER_IN_DEPOSIT is
  // specifically "cash, checks, and ATM deposits," and
  // TRANSFER_IN_CASH_ADVANCES_AND_LOANS is "loans and cash advances
  // deposited into a bank account." TRANSFER_IN_OTHER_TRANSFER_IN and
  // TRANSFER_OUT_OTHER_TRANSFER_OUT are literally Plaid's catch-all —
  // "other miscellaneous inbound/outbound transactions" — a real
  // example being a generic ACH direct deposit whose merchant name is
  // just "DEPOSIT," with nothing identifying it as a movement between
  // two of the user's own accounts. TRANSFER_IN_ACCOUNT_TRANSFER
  // ("general inbound transfers from another account") is the one
  // that's actually transfer-shaped and stays classified as a transfer
  // below. Checked first, same reasoning as the credit-card-payment
  // override right after it.
  if (
    detailed === 'TRANSFER_IN_DEPOSIT' || detailed === 'TRANSFER_IN_CASH_ADVANCES_AND_LOANS'
    || detailed === 'TRANSFER_IN_OTHER_TRANSFER_IN' || detailed === 'TRANSFER_OUT_OTHER_TRANSFER_OUT'
  ) {
    return txn.amount > 0 ? 'expense' : 'income';
  }
...
async function queueDashboardReviews(userId, addedTransactions) {
  ...
  for (const txn of addedTransactions) {
    ...
    const { data: possibleDuplicate } = await supabaseAdmin
      .from('pending_dashboard_reviews')
      .select('id, status, finance_log(id)')
      .eq('user_id', userId)
    ...
    const hasHandledDuplicate = (possibleDuplicate || []).some(r => r.status === 'dismissed' || (r.finance_log && r.finance_log.length));
    if (hasHandledDuplicate) continue;

    // Same check against permanently-deleted skipped items — those
    // rows don't exist in pending_dashboard_reviews at all anymore, so
    // the query above can't see them. This is what makes "Delete
    // permanently"'s promise ("won't come back even if detected
    // again") actually hold even when Plaid reissues a new
    // transaction_id for the same purchase (pending -> posted).
    const { data: deletedFingerprint } = await supabaseAdmin
      .from('deleted_review_fingerprints')
      .select('id')
      .eq('user_id', userId)
      .eq('linked_account_id', linkedAccountId)
      .eq('amount', Math.abs(txn.amount))
      ...
      .lte('txn_date', windowEnd.toISOString().slice(0, 10))
      .limit(1);
    if (deletedFingerprint && deletedFingerprint.length) continue;
...
//
// Gated by the shared once-a-day sync budget (see isSyncDue above) — if
// this item already had a live Plaid call (of any kind) within the last
// 24 hours, this returns immediately with skipped: true instead of
// calling Plaid again.
async function refreshTransactionsForItem(itemRow, { force = false } = {}) {
  if (!isSyncDue(itemRow, { force })) {
    return { addedCount: 0, dashboardQueuedCount: 0, merchantQueuedCount: 0, skipped: true, nextSyncAt: nextSyncAt(itemRow, { force }) };
  }

  const { addedTransactions: rawAdded, modifiedTransactions, removedTransactions } = await syncTransactionsForItem(itemRow);
  await markItemSynced(itemRow.item_id);
  ...
  return { addedCount: storageEligible.length, dashboardQueuedCount, merchantQueuedCount, skipped: false };
}
...
//
// Gated by the same shared once-a-day sync budget as
// refreshTransactionsForItem — see isSyncDue above.
async function refreshSubscriptionsForItem(itemRow, { force = false } = {}) {
  if (!isSyncDue(itemRow, { force })) {
    return { streamCount: 0, queuedCount: 0, skipped: true, nextSyncAt: nextSyncAt(itemRow, { force }) };
  }
  ...
  return { streamCount: streams.length, queuedCount, skipped: false };
}
...
// the other. Both halves already respect the shared sync gate
// individually, so this needs no gate logic of its own — it just
// bubbles up whether the item was actually due. `force` is threaded
// straight through to both halves — see FORCE_SYNC_MIN_INTERVAL_MS
// above for what it actually relaxes.
async function processItemUpdate(itemRow, { force = false } = {}) {
  const txnResult = await refreshTransactionsForItem(itemRow, { force });
  const subResult = await refreshSubscriptionsForItem(itemRow, { force });
  return {
