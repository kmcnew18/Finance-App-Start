...
async function refreshRecurringForItem(itemRow) {
  ...
  for (const s of streams) {
    ...
    // Only overwrite `ignored` in the tombstoned direction — never
    // silently un-ignore something the user deliberately hid, but do
    // hide anything Plaid now says is no longer active. A stream that's
    // simply still active shouldn't have this field touched either way.
    if (isTombstoned) fields.ignored = true;
