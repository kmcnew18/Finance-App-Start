...
async function refreshTransactionsForItem(itemRow) {
  ...
  // regardless of when Plaid actually delivers it, and regardless of
  // whether it came from the cursor sync or the supplementary fetch.
...
async function refreshSubscriptionsForItem(itemRow) {
  ...
  if (linkedAccountIds.length) {
    ...
    if (connectedDate) query = query.gte('txn_date', connectedDate);
